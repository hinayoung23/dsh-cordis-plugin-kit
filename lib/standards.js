...
const rules = [
  ...
  ['AUT001', 'error', 'automation', 'balanced 自动化必须包含保存检查、pre-commit、pre-push、prepack 和代理交付指令，且不得在保存阶段执行目标插件代码。'],
  ['AUT002', 'error', 'automation', 'GitHub、GitLab、Gitee 或通用 CI 适配必须调用同一个平台无关质量门，并保留一致的退出码和报告语义。'],
]
...
export const STANDARDS = Object.freeze(rules.map(([id, severity, category, requirement]) => Object.freeze({
  ...
  source: id.startsWith('CRD')
    ...
    : id.startsWith('AUT')
      ? 'Cordis Kit checkpoint and platform-neutral CI contracts'
